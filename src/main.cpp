...
enum class StorageState { MOUNTED, FAILED, ABSENT };

enum class StorageStatus { OK, UNMOUNTED, ABSENT };
...
void storage_unmount() { fs_unmount(&fs_mnt); }
...
//   storage_status_timer();
  ...
  lv_timer_t *timer = lv_timer_create(storage_status_timer, 500, nullptr);
